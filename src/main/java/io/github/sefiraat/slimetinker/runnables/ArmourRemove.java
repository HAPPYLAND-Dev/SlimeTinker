...
public class ArmourRemove extends BukkitRunnable {
    ...
    private void unequip(@Nonnull Player player, @Nonnull ItemStack itemStack) {
        ...
        if (i.firstEmpty() > -1) {
            i.addItem(newItem);
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "你的某件防具已损坏! 它已自动卸到你的物品栏中.");
        } else {
            player.getWorld().dropItemNaturally(player.getLocation(), newItem);
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "你的某件防具已损坏! 你的物品栏已满,它掉在地上了.");
        }
    }
}
