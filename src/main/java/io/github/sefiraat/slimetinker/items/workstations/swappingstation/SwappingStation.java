...
public class SwappingStation extends MenuBlock {
    ...
    protected void craft(BlockMenu blockMenu, Player player) {
        ...
        // No tool dummy!
        if (item == null) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "在第一格内放入匠魂装备");
            return;
        }

        if (item.getAmount() > 1) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "匠魂装备不能堆叠");
            return;
        }

        if (part == null) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "在第二格内放入替换的部件");
            return;
        }
        ...
        if (ItemUtils.isTool(item)) {
            if (partClass != null && ItemUtils.partIsTool(partClass)) {
                ...
                player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "该部件不能替换到此工具/武器上");
            }
            ...
            if (partClass != null && ItemUtils.partIsArmour(partClass)) {
                ...
                player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "该部件不能替换到此防具上");
            }
        } else {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "第一格内的物品不是匠魂工具/武器/防具");
        }
    }
    ...
    private void swapTool(BlockMenu blockMenu,
        ...
        if (partClass.equals(Ids.HEAD) && (partType != null && !partType.equals(ItemUtils.getToolTypeName(item)))) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "该工具头部与工具类型不一致,无法替换");
            return;
        }
    ...
    private void swapArmour(BlockMenu blockMenu,
        ...
        if (partClass.equals(Ids.PLATE) && (partType != null && !partType.equals(ItemUtils.getArmourTypeName(item)))) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "该盔甲板与防具类型不一致,无法替换");
            return;
        }
