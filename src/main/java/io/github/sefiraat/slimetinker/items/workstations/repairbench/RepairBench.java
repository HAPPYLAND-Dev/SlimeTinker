...
public class RepairBench extends MenuBlock {
    ...
    protected void craft(BlockMenu blockMenu, Player player) {
        ...
        // No item dummy!
        if (item == null) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "在第一格内放入需要修复的装备");
            return;
        }
        ...
        if (kit == null || !RepairkitTemplate.isRepairKit(kit)) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "在第二格内放入修复工具");
            return;
        }
        ...
        if (repairChecks(partMaterial, toolMaterial, armourMaterial, item)) {
            ...
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "修复工具的材质与需要修复的物品的材质不一致");
        }
    }
