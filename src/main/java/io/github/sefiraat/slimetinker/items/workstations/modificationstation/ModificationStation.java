...
public class ModificationStation extends MenuBlock {
    ...
    protected void craft(BlockMenu blockMenu, Player player) {
        ...
        // No item dummy!
        if (item == null) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "在第一格放置匠魂装备");
            return;
        }
        ...
        if (ItemUtils.isTool(item)) {
            ...
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "第一格内的物品不是匠魂装备");
        }
    }
    ...
    private void modTool(BlockMenu blockMenu, Player player, ItemStack item, ItemStack modItem) {
        ...
        if (modItem == null || !Modifications.getModificationListTool().contains(StackUtils.getIdOrType(modItem))) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "需要在第二格内放入有效的模组物品");
            return;
        }
        ...
        if (!mod.getRequirementMap().containsKey(currentLevel + 1)) { // Max level
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "模组已到最大等级");
            return;
        }
        ...
        if (currentAmount <= 0) {
            if (modSlots == 0) { // New mod and no slots
                player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "该匠魂装备没有剩余的模组栏位");
                return;
        ...
        if (!blockMenu.fits(newTool, OUTPUT_SLOT)) {
            player.sendMessage(ThemeUtils.prefix + ThemeUtils.WARNING + "输出栏已满");
            return;
        }
